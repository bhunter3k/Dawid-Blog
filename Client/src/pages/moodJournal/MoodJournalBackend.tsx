...
export const MoodJournalBackend = () => {
	//---States---//
	...
	async function handleJournalFormSubmit(event: FormEvent): Promise<void> {
		...
		//else if to only call the predictStress() function when creating or editing a journal
			...
			const result = await predictStress(true);
			if (result) {
				status = result.stressState;
				prob = result.stressProb;
				newJournal = createNewJournal(status, prob);
			}
		}
	...
	async function predictStress(fetching: boolean) {
		try {
			setLoading(true);

			const predictedJournalEntry = await fetch("https://tpd20seu.projects.cmp.uea.ac.uk/journal/journalEntryPrediction", {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({
			...
			const stressData = await predictedJournalEntry.json();

			setStressState(stressData.predictedJournalEntry.stressState);
			setStressProb(stressData.predictedJournalEntry.stressProb);

			setLoading(false);

			if (fetching) {
				return { "stressState": stressData.predictedJournalEntry.stressState, "stressProb": stressData.predictedJournalEntry.stressProb };
			}
		} catch (error) {
		...
		<>
			...
			{!loading && (
				<>
					...
					<p className="infoP">
						The journal model will make stress predictions in the backend on our server as your device does not support frontend browser
						predictions.
					</p>
							...
							<div id="journalModeContainer">
								...
								{stressState !== "" && stressProb !== "" && (
									<div
										...
										onClick={() => {
											setDialogHeader(
												"If you believe that our model has made an incorrect prediction on your entry, please click the confirm button.\n\nYour entry will be marked with the correct stress status and this entry will be used to help train our model in the future."
											);
