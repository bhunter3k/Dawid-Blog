...
export const MoodSelfieBackend = () => {
	//---States---//
	...
	async function predictStress() {
		try {
			...
			// if (editSelfie) {
			// 	const img = convertBase64ToImgObject();
			// 	console.log(img);
			// }
			...
			formData.append("selfieImageName", "tempSelfie.jpg");
			formData.append("selfieImage", blob, "tempSelfie.jpg");

			const predictedSelfie = await fetch("https://tpd20seu.projects.cmp.uea.ac.uk/selfie/selfiePrediction", {
				method: "POST",
				body: formData,
				credentials: "include",
			});

			const predictedSelfieData = await predictedSelfie.json();

			setStressStatus(predictedSelfieData.predictedSelfie.stressState);
			setStressProb(predictedSelfieData.predictedSelfie.stressProb);

			setLoading(false);

			if (editSelfie) {
				return [predictedSelfieData.predictedSelfie.stressState, predictedSelfieData.predictedSelfie.stressProb];
			}
		} catch (error) {
		...
		<>
			...
			{!loading && (
				<>
					...
					<p className="infoP">
						The selfie model will make stress predictions in the backend on our server as your device does not support frontend predictions
					</p>
